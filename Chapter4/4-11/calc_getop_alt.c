 ...
 * Упражнение 4.11, страница 96
 * 
 * Modify getop so that it doesn't need to use ungetch . Hint: use an internal
 * static variable.
 *
 * Измените функцию getop так, чтобы ей не нужно было использовать ungetch.
 * Подсказка: воспользуйтесь внутренней статической переменной.
 */
